...
const emailSchema = z.string().email().refine((email) => !email.includes('@example.com'), { message: 'Please use a real email address' });
...
const timestampSchema = z.number().int().positive().refine((ts) => ts <= Date.now() + 86400000, // Max 1 day in future
